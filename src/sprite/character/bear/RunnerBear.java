...
public class RunnerBear  extends Bear{
	
	public RunnerBear(double x, double y) {
		super();
		setPosition(new Point2D(x*36.0D, y*36.0D));
		speed = 40;
		health = (int) (100*GameStatus.getInstance().getMonsterMultiplier());
		maxHealth = (int) (100*GameStatus.getInstance().getMonsterMultiplier());
		characterAttack = 50;
	...
	public void draw(GraphicsContext gc) {
		...
		Image image ;
		if(imgSide == 0) {
			image = (ImageHolder.getInstance()).runnerBearLeft;
		}else {
			image = (ImageHolder.getInstance()).runnerBearRight;
		}
